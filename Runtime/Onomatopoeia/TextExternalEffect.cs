...
namespace Sainna.Onomatopoeia
{
    public class TextExternalEffect : MonoBehaviour
    {
        ...
        [SerializeField] TextMeshProAnimations[] _TMAnims = null;

        [SerializeField] bool _CancelOtherAnims = false;

        [SerializeField] float _FontSizeMultiplier = -1.0f;

        [SerializeField] float _AnimSpeedMultiplier = -1.0f;

        [SerializeField] Color _NewTextColor = Color.white;

        [SerializeField] Capitalization _TextCapitalization = Capitalization.Inherit;

        [SerializeField] FontStyleEffect _FontStyle = FontStyleEffect.Inherit;

        [Header("Font & Materials")] [SerializeField]
        TMP_FontAsset _OverrideFont = null;

        [SerializeField] Material _OverrideMaterial = null;

        [Header("Speech bubble")] [SerializeField]
        SpeechBubbleActivation _SpeechBubbleActivation = SpeechBubbleActivation.Inherit;

        [SerializeField] SpeechBubble _SpeechBubble = null;


        [Header("Particle system")] [SerializeField]
        ParticleSystem _ParticleSystem = null;

        [SerializeField] float _ParticleSystemCut = -1.0f;
        ...
        void Start()
        {
            ...
            else
            {
                ...
                {
                    TextOwner.ForceCapitalization(_TextCapitalization == Capitalization.Lowercase
                        ? TMPro.FontStyles.LowerCase
                        : TMPro.FontStyles.UpperCase);
                }

                if (_FontStyle != FontStyleEffect.Inherit)
                {
                    TextOwner.AddFontStyle(_FontStyle == FontStyleEffect.Bold
        ...
        public void CutParticleSystem(TMP_Text textComp, TMP_CharacterInfo charInfo, float normalizedAnimProgress,
