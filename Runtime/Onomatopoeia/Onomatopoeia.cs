...
namespace Sainna.Onomatopoeia
{
    ...
    public class Onomatopoeia : MonoBehaviour
    {
        ...
        [Header("Base data")] [SerializeField] TMP_Text _TMPro = null;

        public TMP_Text TMPText => _TMPro;

        [SerializeField] Color _TextColor = Color.white;

        [SerializeField] Vector2 _RandomDirectionBoundary = new Vector2(-0.5f, 0.5f);

        [SerializeField] bool _EnableBaseAnim = true;

        [SerializeField] bool _AlwaysFacePlayer = false;

        FacePlayerType FacePlayerInfo = FacePlayerType.None;

        [SerializeField] bool _DestroyOnAnimEnd = true;

        [SerializeField] float _OnAnimEndDelay = 0;

        [SerializeField] Onomatopoeia _NextOnomatopoeia = null;

        [SerializeField] UnityEvent _OnAnimEnd = null;

        [SerializeField] AnimationCurve _climbAnim = null;

        [SerializeField] float _baseHeight = 1.0f;

        [SerializeField] float _baseDepth = 1.0f;

        [SerializeField] AnimationCurve _alphaAnim = null;

        [SerializeField] float _baseAnimTime = 1.0f;

        [SerializeField] float _baseFontSize = 3.0f;

        [SerializeField] TextMeshProAnimations.AnimEnum _TMAnim = 0;

        [SerializeField] Vector2 _AllowedDisplacementOnX = Vector2.zero;

        [SerializeField,
         Tooltip(
             "The X displacement is applied locally to the text. warning: also used to change DestroyOnAnimEnd behaviour")]
        bool _XDisplacementOnLocal = true; //warning: also used to change DestroyOnAnimEnd behaviour

        Quaternion InitialRotation = Quaternion.identity;

        [SerializeField] bool _AllowMirrorOnX = false;

        [SerializeField] TextMeshProAnimations[] _AnimRefs =
            new TextMeshProAnimations[System.Enum.GetNames(typeof(TextMeshProAnimations.AnimEnum)).Length];


        [Header("Speech bubble options")] [SerializeField]
        SpeechBubble[] _SpeechBubbles = null;

        [SerializeField] bool _EnableSpeechBubble = false;
        [SerializeField] bool _EnableSpeechBubbleAnim = true;

        [SerializeField] bool _FreezeBubbleXScale = false;
        [SerializeField] bool _FreezeBubbleYScale = false;

        [SerializeField] AnimationCurve _SpeechBubbleSizeCurve = null;
        ...
        [SerializeField] AnimFrameEvent _FrameEvents = null;


        [Header("Audio options (if no ColliderImpactSound)")] [SerializeField]
        ...
        [SerializeField] bool _UseSoundProperty = false;
        ...
        public delegate void OptAnimCharLoop(TMP_Text textComp, TMP_CharacterInfo charInfo,
            float normalizedAnimProgress, ref Vector3[] vertices);
        ...
        protected struct AnimationDelegate
        {
            ...
            public void AddDelegates(OptAnimsSetup setup, OptAnimsLoopSetup start, OptAnimCharLoop charLoop,
                OptAnimsLoopSetup end)
            {
                Setup += setup;
                Start += start;
        ...
        public void AddExternalAnimation(TextMeshProAnimations animToAdd)
        {
            ...
            ExternalAnims.AddDelegates(animToAdd.AnimationSetup, animToAdd.AnimationStart, animToAdd.AnimationLoop,
                animToAdd.AnimationEnd);
        ...
        private void BaseAnimation()
        {
            ...
            else
            {
                ...
                //todo: remove new
                _TMPro.rectTransform.parent.localPosition = new Vector3(initPosition.x,
                    initPosition.y + Mathf.Lerp(0, Height,
                        (_climbAnim.Evaluate(currentTimeNormalized * SpeedCoefficient))),
                    initPosition.z + Mathf.Lerp(0, Depth,
                        (_climbAnim.Evaluate(currentTimeNormalized * SpeedCoefficient))));
            }
        }
        ...
        private void WordAnimation()
        {
            ...
            if (currentTimeNormalized < 1.0f)
            {
                ...
                if (SelectedSpeechBubble)
                {
                    float newX = initScaleBubble.x * SpeechBubbleScaleModifierTotal.x * (_FreezeBubbleXScale
                        ? 1.0f
                        : _SpeechBubbleSizeCurve.Evaluate(currentTimeNormalized * SpeedCoefficient));
                    float newY = initScaleBubble.y * SpeechBubbleScaleModifierTotal.y * (_FreezeBubbleYScale
                        ? 1.0f
                        : _SpeechBubbleSizeCurve.Evaluate(currentTimeNormalized * SpeedCoefficient));
                    ...
                    SelectedSpeechBubble.transform.localScale = new Vector3(
                        newX,
                        newY,
                        initScaleBubble.z * SpeechBubbleScaleModifierTotal.z *
                        _SpeechBubbleSizeCurve.Evaluate(currentTimeNormalized * SpeedCoefficient)
                    );
        ...
        public void TextFacePlayer(Transform player, FacePlayerType faceType = FacePlayerType.AllDirection,
            bool firstCall = true)
        {
            if (player)
            {
        ...
        {
            if (_TMPro.rectTransform.parent == null)
            {
                Debug.LogWarning("Please add a parent to the Onomatopoeia script. You risk encountering buggy behaviour.");
                initPosition = _TMPro.rectTransform.localPosition;
            }
            else
            {
        ...
        public void SetupAnimation(bool alsoReset = false)
        {
            ...
            {
                var values =
                    (TextMeshProAnimations.AnimEnum[]) System.Enum.GetValues(typeof(TextMeshProAnimations.AnimEnum));
                ...
                for (int i = 0; i < values.Length; i++)
                {
                    ...
                    {
                        OptAnims.AddDelegates(_AnimRefs[i].AnimationSetup, _AnimRefs[i].AnimationStart,
                            _AnimRefs[i].AnimationLoop, _AnimRefs[i].AnimationEnd);
                    }
                }
