...
namespace Sainna.Onomatopoeia
{
    public class BezierSurface : MonoBehaviour
    {
        ...
        private void OnDrawGizmos()
        {
            ...
            for (int i = 0; i < _GizmosPoints; i++)
            {
                ...
                for (int j = 0; j < _GizmosPoints; j++)
                {
                    ...
                    Vector3 point = Vector3.Lerp(_Curves[0].GetBezierPosition(ti), _Curves[1].GetBezierPosition(ti),
                        tj);
        ...
        public OrientedPoint GetOrientedPoint(float tx, float ty)
        {
            ...
            return new OrientedPoint(Vector3.Lerp(p1.point, p2.point, ty),
                Quaternion.Slerp(p1.rotation, p2.rotation, ty));
        }
    }
}